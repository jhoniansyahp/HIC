...
class EJuiAutoCompleteFkField extends CJuiAutoComplete {
	...
	private $_fieldName;
        ...
        /**
	 * @var string the name of the temp field
	 */
	private $_fieldName2;
    ...
    public function init() {
        ...
        $this->_fieldName = get_class($this->model).'_display_'.$this->attribute;  // match what is generated by Yii
        $this->_saveName = get_class($this->model).'_'.$this->attribute;
        ...
        $this->options['select']="js:function(event, ui)
            {
                \$('#".$this->_fieldName."').val(ui.item.id);
                \$('#".$this->_saveName."').val(ui.item.value);
                    ".$tempz."
            }";
