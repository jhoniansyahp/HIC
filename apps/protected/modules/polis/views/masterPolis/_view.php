<div class="view">

	<b><?php echo CHtml::encode($data->getAttributeLabel('v_policy_no')); ?>:</b>
	<?php echo CHtml::link(CHtml::encode($data->v_policy_no),array('view','id'=>$data->v_policy_no)); ?>
	<br />

	<b><?php echo CHtml::encode($data->getAttributeLabel('d_start')); ?>:</b>
	<?php echo CHtml::encode($data->d_start); ?>
	<br />

	<b><?php echo CHtml::encode($data->getAttributeLabel('v_plan_code')); ?>:</b>
	<?php echo CHtml::encode($data->v_plan_code); ?>
	<br />

	<b><?php echo CHtml::encode($data->getAttributeLabel('v_currency')); ?>:</b>
	<?php echo CHtml::encode($data->v_currency); ?>
	<br />

	<b><?php echo CHtml::encode($data->getAttributeLabel('v_company_code')); ?>:</b>
	<?php echo CHtml::encode($data->v_company_code); ?>
	<br />

	<b><?php echo CHtml::encode($data->getAttributeLabel('n_agent_no')); ?>:</b>
	<?php echo CHtml::encode($data->n_agent_no); ?>
	<br />

	<b><?php echo CHtml::encode($data->getAttributeLabel('v_contact_person')); ?>:</b>
	<?php echo CHtml::encode($data->v_contact_person); ?>
	<br />

	<?php /*
	<b><?php echo CHtml::encode($data->getAttributeLabel('v_contact_telp')); ?>:</b>
	<?php echo CHtml::encode($data->v_contact_telp); ?>
	<br />

	<b><?php echo CHtml::encode($data->getAttributeLabel('v_address')); ?>:</b>
	<?php echo CHtml::encode($data->v_address); ?>
	<br />

	<b><?php echo CHtml::encode($data->getAttributeLabel('v_city')); ?>:</b>
	<?php echo CHtml::encode($data->v_city); ?>
	<br />

	<b><?php echo CHtml::encode($data->getAttributeLabel('v_policy_status')); ?>:</b>
	<?php echo CHtml::encode($data->v_policy_status); ?>
	<br />

	<b><?php echo CHtml::encode($data->getAttributeLabel('n_member')); ?>:</b>
	<?php echo CHtml::encode($data->n_member); ?>
	<br />

	<b><?php echo CHtml::encode($data->getAttributeLabel('v_premi_freq')); ?>:</b>
	<?php echo CHtml::encode($data->v_premi_freq); ?>
	<br />

	<b><?php echo CHtml::encode($data->getAttributeLabel('n_premi_budget')); ?>:</b>
	<?php echo CHtml::encode($data->n_premi_budget); ?>
	<br />

	<b><?php echo CHtml::encode($data->getAttributeLabel('v_add_info_penanggung')); ?>:</b>
	<?php echo CHtml::encode($data->v_add_info_penanggung); ?>
	<br />

	<b><?php echo CHtml::encode($data->getAttributeLabel('n_add_info_premi')); ?>:</b>
	<?php echo CHtml::encode($data->n_add_info_premi); ?>
	<br />

	<b><?php echo CHtml::encode($data->getAttributeLabel('v_upload_by')); ?>:</b>
	<?php echo CHtml::encode($data->v_upload_by); ?>
	<br />

	<b><?php echo CHtml::encode($data->getAttributeLabel('d_upload_date')); ?>:</b>
	<?php echo CHtml::encode($data->d_upload_date); ?>
	<br />

	<b><?php echo CHtml::encode($data->getAttributeLabel('v_verifikasi_by')); ?>:</b>
	<?php echo CHtml::encode($data->v_verifikasi_by); ?>
	<br />

	<b><?php echo CHtml::encode($data->getAttributeLabel('d_verifikasi_date')); ?>:</b>
	<?php echo CHtml::encode($data->d_verifikasi_date); ?>
	<br />

	<b><?php echo CHtml::encode($data->getAttributeLabel('v_created_by')); ?>:</b>
	<?php echo CHtml::encode($data->v_created_by); ?>
	<br />

	<b><?php echo CHtml::encode($data->getAttributeLabel('d_created_date')); ?>:</b>
	<?php echo CHtml::encode($data->d_created_date); ?>
	<br />

	<b><?php echo CHtml::encode($data->getAttributeLabel('v_updated_by')); ?>:</b>
	<?php echo CHtml::encode($data->v_updated_by); ?>
	<br />

	<b><?php echo CHtml::encode($data->getAttributeLabel('d_updated_date')); ?>:</b>
	<?php echo CHtml::encode($data->d_updated_date); ?>
	<br />

	*/ ?>

</div>